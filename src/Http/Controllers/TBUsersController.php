...
class TBUsersController extends Controller
{
    ...
    {
        $isAllow = Config::get('builder::users.check.users.update');
        if (!$isAllow()) {
            App::abort(404);
        }
    ...
    {
        $isAllow = Config::get('builder::users.check.groups.update');
        if (!$isAllow()) {
            App::abort(404);
        }
    ...
    {
        $isAllow = Config::get('builder::users.check.users.create');
        if (!$isAllow()) {
            App::abort(404);
        }
    ...
    {
        $isAllow = Config::get('builder::users.check.groups.create');
        if (!$isAllow()) {
            App::abort(404);
        }
    ...
    public function doCreateGroup()
    {
        ...
        $total = 0;
        $perms = Config::get('builder::users.permissions', array());
    ...
    {
        $isAllow = Config::get('builder::users.check.users.delete');
        if (!$isAllow()) {
            App::abort(404);
        }
