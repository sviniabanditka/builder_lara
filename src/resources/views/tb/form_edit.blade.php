        ...
        @else
            ...
            <ul class="nav nav-tabs bordered">
                @foreach ($def['position']['tabs'] as $title => $fields)
                    <li @if ($loop->first) class="active" @endif><a href="#etabform-{{$def['db']['table']}}-{{$loop->index1}}" data-toggle="tab">{{ __cms($title) }}</a></li>
                @endforeach
            </ul>
            <div class="tab-content padding-10">
                @foreach ($def['position']['tabs'] as $title => $fields)
                    <div class="tab-pane @if ($loop->first) active @endif" id="etabform-{{$def['db']['table']}}-{{$loop->index1}}">
